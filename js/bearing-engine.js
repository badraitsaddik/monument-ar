...
function filterCandidates(heading) {
  ...
  return _monuments
    .map((m) => {
      const bearing = computeBearing(_userLat, _userLon, m.lat, m.lon);
      const delta   = angularDelta(heading, bearing);
      const dist    = haversineDistance(_userLat, _userLon, m.lat, m.lon);
      return { ...m, bearing, delta, dist };
    })
    .filter((m) => Math.abs(m.delta) <= CONFIG.THRESHOLD_DEG && m.dist <= (m.distance_max_m || CONFIG.MAX_DIST_M))
...
function getDirectionMessage(monument, phoneIsRaised = true) {
  ...
  const side    = delta > 0 ? 'droite' : 'gauche';
  const degrees = Math.abs(Math.round(delta));
  return { text: nom + ' est à votre ' + side, detail: 'Tournez de ' + degrees + '°', dist: distStr, status: delta > 0 ? 'right' : 'left' };
}

// ── 12. Boucle principale ────────────────────────────────────────────────────
async function startBearingEngine({ onUpdate, onGPSError }) {
  await loadMonuments();
  startGPS(
    () => {
      if (_smoothedHeading !== null) {
